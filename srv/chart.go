...
func HandleChart(ctx context.Context, args []string) {
	...
	console.Log("[chart] %s / %s / %s~%s", opts.source, opts.symbol, opts.from, opts.to)
	...
	if opts.telegram {
		caption := fmt.Sprintf("📈 %s %s (%s~%s)", opts.source, opts.symbol, opts.from, opts.to)
...
func parseChartArgs(args []string) (chartOptions, error) {
	...
	for _, d := range []struct{ name, v string }{{"--from", opts.from}, {"--to", opts.to}} {
		if _, err := time.Parse("20060102", d.v); err != nil {
	...
	if opts.out == "" {
		...
		opts.out = filepath.Join("test_logs/charts",
			fmt.Sprintf("%s_%s_%s-%s.png", opts.source, opts.symbol, opts.from, opts.to))
	}
	return opts, nil
}
