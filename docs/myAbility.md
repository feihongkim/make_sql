...
### Telegram MTProto를 통한 메시지 전송
Telethon (사용자 계정 세션)으로 각 Docker Claude 봇에 **사용자 입력과 동일한 메시지**를 전송할 수 있다.
- 전송만: `tg_send.py <bot_username> <message>`
- 전송 + 응답 수신: `tg_send.py --ask <bot_username> <message>`

기존 Docker Claude의 세션/컨텍스트가 유지된 상태에서 작업을 요청하고 결과를 회수할 수 있다.
...
### Headless 명령 실행 (abledb send)
기존 세션과 무관하게 1회성 프롬프트를 실행하고 결과를 받을 수 있다.
...
2. MakeSQL Claude가 적합한 Docker Claude에 작업 위임 (tg_send.py)
