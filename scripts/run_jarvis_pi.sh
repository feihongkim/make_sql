#!/bin/bash
# jarvis_pi 컨테이너 구동 스크립트 (호스트: white)
#
# 사용법:
#   ./scripts/run_jarvis_pi.sh
...
CONTAINER_NAME="jarvis_pi"
IMAGE_NAME="api_pi:11"
DATA_VOLUME="jarvis_pi_data_v1"
WORKSPACE_DIR="/home/feihong/code/Jarvis"
TELEGRAM_DIR="${WORKSPACE_DIR}/.telegram"
TELEGRAM_EXTENSION_SOURCE_CONTAINER="${TELEGRAM_EXTENSION_SOURCE_CONTAINER:-rstudio_pi}"

: "${JARVIS_PI_TELEGRAM_BOT_TOKEN:?JARVIS_PI_TELEGRAM_BOT_TOKEN missing in ~/.api_keys}"
...
docker run -d \
  ...
  -e TELEGRAM_BOT_TOKEN="${JARVIS_PI_TELEGRAM_BOT_TOKEN}" \
...
# api_pi:11 + fresh /home/node volume 조합에서는 Telegram extension이 빠질 수 있다.
# 정상 컨테이너(rstudio_pi 등)에서 extension을 복사해 bootstrap한다.
